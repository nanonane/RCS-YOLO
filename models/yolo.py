...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='../cfg/training/yolo-exp.yaml', help='model.yaml')
    ...
    # Create model
    model = Model(cfg = opt.cfg, img_size=opt.img_size).to(device)
    model.eval()
    ...
    img = torch.rand(1, 3, opt.img_size, opt.img_size).to(device)
    model(img)

    with torch.no_grad():
        model.fuse(opt.img_size)
        t0 = time_synchronized()
        for _ in range(100):
            model(img)
        print(f"{(time_synchronized() - t0) * 10:.2f}ms")
